...
def contact(request):
    if request.method == 'GET':
        ...
        new_data = Contact_Query(name=a, email=b, message=d)
        new_data.save()
...
def loginuser(request):
    if request.method == 'GET':
        ...
        if user is None:
            return render(request, 'test1/loginuser.html', {'form': AuthenticationForm(), 'error':' Invaild Credentials'})
        else:
            login(request, user)
